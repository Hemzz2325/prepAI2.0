...
function QuestionsSection({ mockInterviewQuestion, activeQuestionIndex }) {
  ...
  return mockInterviewQuestion && (
    <div className='p-6 border rounded-2xl shadow-sm bg-white dark:bg-gray-900 h-full flex flex-col max-h-[calc(100vh-200px)]'>
      {/* Progress Grid */}
      ...
      <div className="flex-grow flex flex-col justify-center overflow-y-auto">
        <h2 className='text-lg md:text-xl font-bold text-gray-800 dark:text-gray-100 leading-relaxed animate-in fade-in slide-in-from-left-4 duration-500'>
          {mockInterviewQuestion[activeQuestionIndex]?.question}
        </h2>

        <div className="mt-4 flex items-center gap-3">
          <div
            className='p-2 bg-blue-50 dark:bg-blue-900/20 rounded-full cursor-pointer hover:bg-blue-100 dark:bg-blue-900/30 transition-colors'
            onClick={() => textToSpeach(mockInterviewQuestion[activeQuestionIndex]?.question)}
            title="Read Question"
          >
            <Volume2 className='text-blue-600 h-5 w-5' />
          </div>
          <span className="text-xs text-gray-400 dark:text-gray-500">Click to listen</span>
        </div>
      </div>

      {/* Note Section */}
      <div className='border-l-4 border-blue-500 bg-blue-50 dark:bg-blue-900/20 p-3 rounded-r-lg mt-4'>
