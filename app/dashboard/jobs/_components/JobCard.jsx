...
function JobCard({ job, onStatusChange, onDelete, statuses }) {
    ...
    return (
        <motion.div
            ...
            className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm hover:shadow-md transition-all relative group"
        >
            {/* Menu Button */}
            ...
            {/* Company & Role */}
            <h4 className="font-bold text-gray-900 text-sm mb-1 pr-8">{job.companyName}</h4>
            <p className="text-xs text-gray-600 mb-3">{job.jobRole}</p>
            ...
            <div className="mt-3 pt-3 border-t border-gray-100">
                <select
                    ...
                    className="w-full text-xs px-2 py-1 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent cursor-pointer"
                >
                    {statuses.map((status) => (
