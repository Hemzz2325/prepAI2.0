...
export async function applyRateLimit(req) {
  ...
  try {
    ...
    console.warn("Rate limiting failed or Redis is not configured. Proceeding without rate limit. Error:", error.message);
  }

  return null; // no rate limit exceeded (or failed gracefully) → proceed normally
}
